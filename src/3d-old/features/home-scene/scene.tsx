...
import { BufferAttribute, BufferGeometry, Color, LineSegments, MathUtils, Points } from "three";
...
function GridRelief() {
  ...
  const { positions2D, segments, heights } = useGridData();
  ...
  useFrame(() => {
    ...
    if (pointsRef.current) {
      ...
      pointsRef.current.position.z = posZ;
      pointsRef.current.rotation.x = rotX;
      // pointsRef.current.material.opacity = dotOpacity
    }
